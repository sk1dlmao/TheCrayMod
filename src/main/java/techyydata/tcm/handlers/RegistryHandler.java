        ...
        public static void registerItems(RegistryEvent.Register<Item> event) {
            ...
            for (Item item : items) {
                if (item.getRegistryName() != null) {
                    reg.register(item);
                    ITEM_LIST.add(item);
                    Main.getLogger().info("Successfully added items: " + item.getRegistryName().getResourcePath() + " to the game.");
                } else
                    Main.getLogger().warn("Tried to register items without registry name, ignoring.");
            }
        }
